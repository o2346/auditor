...
```bash
sam-app-eventbridge$ sam build --use-container
```
...
```bash
sam-app-eventbridge$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
sam-app-eventbridge$ sam logs -n HelloWorldFunction --stack-name sam-app-eventbridge --tail
```
...
```bash
sam-app-eventbridge$ pip install pytest pytest-mock --user
sam-app-eventbridge$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name sam-app-eventbridge
```
