...
```bash
cavt$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
cavt$ sam logs -n HelloWorldFunction --stack-name cavt --tail
```
...
```bash
cavt$ pip install pytest pytest-mock --user
cavt$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name cavt
```
