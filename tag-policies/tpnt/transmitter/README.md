...
```bash
transmitter$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
transmitter$ sam logs -n HelloWorldFunction --stack-name transmitter --tail
```
...
```bash
transmitter$ pip install pytest pytest-mock --user
transmitter$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name transmitter
```
