...
```bash
generator$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
generator$ sam logs -n HelloWorldFunction --stack-name generator --tail
```
...
```bash
generator$ pip install pytest pytest-mock --user
generator$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name generator
```
